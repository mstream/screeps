...
describe("TaskFactory", () => {
    ...
    describe("constructor", () => {
        ...
        it("throws exception during taskFactory creation when task taskCosts is null", () => {
            expect(() => new TaskFactory(null)).to.throw("taskCosts can't be null");
        });

        it("throws exception during taskFactory creation when task type is undefined", () => {
            expect(() => new TaskFactory(undefined).to.throw("taskCosts can't be null"));
        });
    });
    ...
    describe("#fromJSON()", () => {
        ...
        it("fromJSONs exits computing tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.EXITS_COMPUTING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.EXITS_COMPUTING, options: {foo: "bar"}});
        ...
        it("fromJSONs extensions building tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.EXTENSIONS_BUILDING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.EXTENSIONS_BUILDING, options: {foo: "bar"}});
        ...
        it("fromJSONs extensions computing tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.EXTENSIONS_COMPUTING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.EXTENSIONS_COMPUTING, options: {foo: "bar"}});
        ...
        it("fromJSONs road computing tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.ROAD_COMPUTING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.ROAD_COMPUTING, options: {foo: "bar"}});
        ...
        it("fromJSONs roads building tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.ROADS_BUILDING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.ROADS_BUILDING, options: {foo: "bar"}});
        ...
        it("fromJSONs walls computing tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.WALLS_COMPUTING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.WALLS_COMPUTING, options: {foo: "bar"}});
        ...
        it("fromJSONs walls building tasks", () => {
            const taskFactory = new TaskFactory({[taskTypes.WALLS_BUILDING]: 1});
            const task = taskFactory.fromJSON({type: taskTypes.WALLS_BUILDING, options: {foo: "bar"}});
    ...
    describe("#create", () => {
        ...
        it("throws exception during task creation when task type is null", () => {
            expect(
                () => new TaskFactory({}).create(null, {})
            ).to.throw("type can't be null");
        });

        it("throws exception during task creation when task type is undefined", () => {
            expect(
                () => new TaskFactory({}).create(undefined, {})
            ).to.throw("type can't be null");
        });

        it("throws exception during task creation when task type is unknown", () => {
            expect(
                () => new TaskFactory({"unknown": 1}).create("unknown", {})
            ).to.throw("unknown task type: unknown");
        });

        it("throws exception during task creation when task type is unknown", () => {
            expect(
                () => new TaskFactory({[taskTypes.ROADS_BUILDING]: 1}).create(taskTypes.WALLS_BUILDING, {})
            ).to.throw(`undefined cost for task type: ${taskTypes.WALLS_BUILDING}`);
        });
